...
def print_model_accurecy_loss_functions(history):
...
def calculate_score(id_train, X_train, y_train):
    ...
    for threshold in np.arange(0.5, 1.0, 0.05):
        ...
        for count_image in  range(len(id_train)):
            ...
            sample_pred_cv_flat =_reshape_image(preds_cv_original_size[count_image])
            sample_real_cv_flat = _reshape_image(y_train_orginal[count_image])
            iou_mean += _calculate_scoring(sample_real_cv_flat,sample_pred_cv_flat)
...
for count_image in range(len(preds_test)):
    ...
    preds_test_org[count_image] =  _resize_image_size((preds_test[count_image]> threshold).astype(int),
                  img_size_original) 
